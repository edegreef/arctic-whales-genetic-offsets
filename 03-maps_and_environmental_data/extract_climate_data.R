# Script for extracting environmental/climate data through sdmpredictors. 

# For a quick plot, can follow: https://www.bio-oracle.org/code.php
# I used a lot of help from Tom Jenkin's tutorial for plotting and extracting points for multiple layers: https://tomjenkins.netlify.app/tutorials/r-extract-marine-data/ 

# This script is divided into four main steps. The starting lines for each step are listed below.
# Step 1) Test run with sdmpredictors, and also set up crop boundary (start at line 31)
# Step 2) Download climate data rasters/layers (start at line 86)
# Step 3) Make nicer plots and also one for difference between present/future (start at line 156)
# Step 4) Extract data from rasters for each sampling location (start at line 348) and also put in format for data by individual (start at line 406)
...
# Additional packages specifically for Step 3 for maps/plots
...
library(colorspace) # Zeileis et al., 2020)

# For the beeswarm plot
library(ggbeeswarm) # (Clark et al., 2023)

######### Step 1) Start in sdmpredictors and do a test run
# Take a quick look at layers. set marine=TRUE to look at only marine data
list_layers(marine=TRUE)

# It's a very long list, but can save as a csv to save and look at later
list <- list_layers(marine=TRUE)
write.csv(list, "list_layers_marine.csv") 

# Here going to use variables with "BO22" for the most updated version
...
## Skip to line 90 if choosing specific variables
...
# Select variables
# For Arctic whales, choosing 4 variables (sst, ice, salinity, chloro): 
...
# Noting that for some reason the future projections for chlorophyll have different extent (ymin -79.5 instead of -90. not relevant for this project but annoying for data upload)
...
# Combine present-day and future vectors - chlorophyll separate here but just because of the difference in extent- will crop them all to be the same shortly.
data <- c(present.vars, future.vars.60, future.vars.85)
...
# Tip: to avoid re-loading these layers in the future, can save in directory, example here:
#data.rasters <- load_layers(data, datadir="C:/Users/eveli/Dropbox/Whales with Garroway/01-arctic_whales/gradient_forest_current/climate_rasters/")

# Then to load in the downloaded rasters (example):
...
######### Step 3) Make nicer plots and also one for difference between present/future. This corresponds to supplemental figures S4, S5.
...
# Make individual plot for each environmental variable, then combine after with patchwork
SST <- ggplot()+
...
# Combine in two parts first
patchwork1 = SST+ICE 
patchwork2 = SAL+CHLO

# Combine and merge axes. This is probably not the most elegant way, but it works (to reduce the redundancy of axes titles)
...
# Load site locations (needs coordinates for each sampling/target site). Using same sample location info for "combine_rangemaps.R"
...
# Quick plot with points on map with each of the rasters
...
# Extract data for each layer. Here adding a buffer (radius in metres) to select mean value for each site location, setting buffer to 100000 (for 100km buffer).
store_data = list()
...
# Set up climate.data dataframe for to save extracted data
...
# Save climate.data by site
write_csv(climate.data, "climatedata.temp.ice.sal.chlo.100KBUFFER.allsp.csv")

### Save climate data by individuals (beluga example:)
# Load sample metadata ## ids need to be in matching order of vcf

# Lines 410-412 specifically for beluga file
sample_info <- read.csv("sample_info_beluga.csv")
sample_info <- subset(sample_info, final_vcf=="yes")
list <- select(sample_info, c("Vcf_ID", "location_ID"))

# Left_join with climate data
climate_data_allsamples <- left_join(list, climate.data, by=c("location_ID"="ID"))

# Save climate data by individual/sample
write_csv(climate_data_allsamples, "env_data_by_sample_beluga.csv")

### Lines 421-431 for narwhal/bowhead files
sample_info <- read.csv("sample_info_narwhal.csv")
sample_info <- subset(sample_info, remove!="X")
list <- select(sample_info, c("sample_ID", "location_ID"))
climate_data_allsamples <- left_join(list, climate.data, by=c("location_ID"="ID"))
write_csv(climate_data_allsamples, "env_data_by_sample_narwhal.csv")

sample_info <- read.csv("sample_info_bowhead.csv")
sample_info <- subset(sample_info, remove!="X")
list <- select(sample_info, c("sample_ID", "location_ID"))
climate_data_allsamples <- left_join(list, climate.data, by=c("location_ID"="ID"))
write_csv(climate_data_allsamples, "env_data_by_sample_bowhead.csv")

##############
## Additional code below to visualize spread of extracted points. This corresponds to Figure S6.
...
# Merge plots together into one
(SST + ICE)/(SAL+CHLO) + plot_layout(guides="collect")

# Save for supplemental figure
ggsave("beeswarm_plots_100KMBUFFER_allsp_size3_latcolor_v2.png", width=7, height=6, dpi=500)
